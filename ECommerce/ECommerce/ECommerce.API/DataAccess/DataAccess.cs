    ...
    public class DataAccess : IDataAccess
    {
                ...
                while (reader.Read())
                {
                    Product product = new Product
                    {
                        ...
                        ProductCategory = new ProductCategory
                        {
                            ...
                            Category = (string)reader["Category"],
                            SubCategory = (string)reader["SubCategory"]
                            // Add other properties of ProductCategory if needed
                        },
                        Offer = new Offer
                        {
                            ...
                            // Add other properties of Offer if needed
                        },
                        Price = (double)reader["Price"],
                        Quantity = (int)reader["Quantity"]
                    };
        ...
        public bool DeleteCartItem(int userId, int cartItemId)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                string query = "SELECT COUNT(*) FROM CartItems WHERE CartItemId = @cartItemId AND CartId IN (SELECT CartId FROM Carts WHERE UserId = @userId AND Ordered = 'false');";
                command.CommandText = query;
